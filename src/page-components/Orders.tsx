...
export default function Orders() {
  ...
  return (
    <>
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "orders" | "preorders")}
      ...
      >
        ...
        <div className="sticky top-0 z-10 bg-background pb-4 space-y-4 border-b border-border mb-4 -mx-4 lg:-mx-6 px-4 lg:px-6 pt-4 lg:pt-6">
          ...
          <TabsList className="bg-muted/60 w-full sm:w-auto">
            ...
            <TabsTrigger value="preorders" className="gap-2 flex-1 sm:flex-none">
              <PackageSearch className="h-3.5 w-3.5" />
              Requested Devices
              {wishes.filter((w) => w.status === "pending").length > 0 && (
                <Badge
                  variant="destructive"
                  className="ml-1 h-5 w-5 p-0 flex items-center justify-center text-[11px] font-semibold rounded-full"
                >
          ...
          {/* Orders filters (only show in orders tab) */}
          {activeTab === "orders" && (
        ...
        <div className="flex-1 overflow-y-auto min-h-0 -mx-4 lg:-mx-6 px-4 lg:px-6">
          ...
          <TabsContent value="preorders" className="mt-0">
            <RequestedDevicesTab
              wishes={wishes}
              userEmails={wishUserEmails}
              isLoading={wishesLoading}
              onViewWish={(wish) => {
                setSelectedWish({ ...wish, userEmail: wishUserEmails[wish.userId] });
